...
def get_boxes_and_labels(
	...
	data.append((dict(ymin=ymin, xmin=xmin, ymax=ymax, xmax=xmax),
		box_to_display_str_map[box],
		color_rgb[color.lower()]))
 
    return data


def drawRects(image, rects):
    for box, name, color in rects:
        ymin, xmin, ymax, xmax = box['ymin'], box['xmin'], box['ymax'], box['xmax'] 
	print color, ' ', name
        vis_util.draw_bounding_box_on_image_array(image, ymin, xmin,
        ymax, xmax, color=color, display_str_list=[name], thickness=2)
